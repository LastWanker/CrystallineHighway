...
系统预设一个低维向量空间，例如 8 维。
每一维不是具体主题，而是一个极粗、稳定的范畴轴，例如：
自然 / 人造、具体 / 抽象、动态 / 静态、主观 / 客观、时间指向、社会性、情绪性、技术性。

这些维度的目的不是精确语义表达，而是提供一个“范畴力场”：
    - 给新建提供方向性（别完全随机）
    - 给长期运行提供某种弱牵引（别彻底漂到不可收敛）
...
11.3 固化后的补充设定（你已经写进旧 README 的那三条）
    固化后的元在构建时，其被寻找的容忍度暂定为各子元容忍度的乘积，因此其数值将非常之大。
    固化后的维度位置暂定为来源子元维度向量的平均值，但整体向更离心、更稀疏的空间方向偏移。
    由于固化元的容忍度极高、触发频率极低，其对应的场系数将非常大，
    因此在语义空间中的位置高度稳定、难以漂移。

11.4 固化层级（贴标签，不是无限增几何维度）
...
12.2 会话态点亮：从 0 开始、可衰减、可维护
    点亮计数是会话内注意力过程：
        新开始一次对话前，各节点点亮可以视作平的（从 0 起步）。
        随对话进行，被触达的地方逐步点亮。
    点亮可以渐衰：
        衰减可基于会话进程，也可基于现实时间，甚至混合（按用途决定）。
    会话结束后可以异步维护：
        本次对话内容再喂回系统继续结晶，这一步不要求与对话同步完成。
